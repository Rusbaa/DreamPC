...
<div class="w-full max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-white">Add New Component</h1>
            <p class="text-sm text-slate-400">Enter component specs, pricing, and initial stock</p>
        </div>
    ...
    <form method="POST" action="{{ route('products.store') }}" class="bg-slate-900/80 border border-slate-800 rounded-2xl p-8 shadow-2xl backdrop-blur-xl space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required
                    class="w-full bg-slate-950 border border-slate-800 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-lg px-4 py-2 text-sm text-white placeholder-slate-600 outline-none transition"
            ...
            <div>
                ...
                <input type="text" name="sku" value="{{ old('sku') }}" required
                    class="w-full bg-slate-950 border border-slate-800 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-lg px-4 py-2 text-sm text-white placeholder-slate-600 outline-none transition"
                    placeholder="e.g. AMD-R7-7800X3D">
            </div>
        ...
        <div>
            ...
            <textarea name="description" rows="3"
                class="w-full bg-slate-950 border border-slate-800 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-lg px-4 py-2 text-sm text-white placeholder-slate-600 outline-none transition"
                placeholder="Product description and details...">{{ old('description') }}</textarea>
        </div>
            ...
            <div id="specs-container" class="space-y-3">
                <div class="flex items-center space-x-3 spec-row">
                    <input type="text" name="specs[0][key]" placeholder="Key (e.g. socket)"
                        class="w-1/2 bg-slate-950 border border-slate-800 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-600 outline-none">
