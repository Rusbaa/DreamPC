...
<div class="max-w-6xl mx-auto space-y-8 pb-12">
    ...
    <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 shadow-xl backdrop-blur-xl flex items-center justify-between">
        ...
        <a href="<?php echo e(route('cart.index')); ?>" class="bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 px-4 py-2 rounded-xl text-xs font-semibold transition">
            ← Return to Cart
        </a>
    </div>
    ...
    <form method="POST" action="<?php echo e(route('checkout.store')); ?>" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <?php echo csrf_field(); ?>
            ...
            <div class="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="customer_name" value="<?php echo e(old('customer_name', Auth::user()->name ?? '')); ?>" 
                    ...
                    <div>
                        ...
                        <input type="text" name="customer_phone" value="<?php echo e(old('customer_phone')); ?>" placeholder="+1 (555) 000-0000" 
                               class="w-full bg-slate-950 border border-slate-800 text-xs text-white rounded-xl px-4 py-3 outline-none focus:border-blue-500" required>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="delivery_date" value="<?php echo e(old('delivery_date', date('Y-m-d', strtotime('+1 day')))); ?>" 
                ...
                <div id="address-container">
                    ...
                    <textarea name="address" rows="3" placeholder="Enter complete street address, city, state, zip code..." 
                              class="w-full bg-slate-950 border border-slate-800 text-xs text-white rounded-xl p-3 outline-none focus:border-blue-500"><?php echo e(old('address')); ?></textarea>
                </div>
            </div>
            ...
            <div class="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6">
                    ...
                    <?php $__currentLoopData = $cart->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="flex items-center justify-between text-xs py-1 border-b border-slate-800/50">
                            <div class="truncate max-w-[70%]">
                                <span class="text-slate-200 font-medium truncate block"><?php echo e($item->product->name); ?></span>
                                <span class="text-[10px] text-slate-400">Qty: <?php echo e($item->quantity); ?></span>
                            </div>
                            <span class="text-emerald-400 font-mono font-bold">$<?php echo e(number_format($item->quantity * $item->unit_price, 2)); ?></span>
                        </div>
                ...
                <div class="space-y-2 text-xs border-t border-slate-800 pt-4">
                    <div class="flex justify-between text-slate-400">
                        <span>Subtotal</span>
                        <span class="font-mono text-slate-200">$<?php echo e(number_format($subtotal, 2)); ?></span>
                    </div>

                    <?php if($appliedCoupon): ?>
                        <div class="flex justify-between text-emerald-400 font-semibold">
                            <span>Coupon (<?php echo e($appliedCoupon['code']); ?>)</span>
                    ...
                    <div class="flex justify-between text-slate-400">
                        <span>Est. Tax (5%)</span>
                        <span class="font-mono text-slate-200">$<?php echo e(number_format($tax, 2)); ?></span>
                    </div>

                    <div class="flex justify-between text-slate-400">
                        <span>Shipping</span>
                        <span class="font-mono text-slate-200">$<?php echo e(number_format($shipping, 2)); ?></span>
                    </div>

                    <div class="pt-3 border-t border-slate-800 flex justify-between text-sm font-bold">
                        <span class="text-white">Total</span>
                        <span class="text-emerald-400 font-mono text-base">$<?php echo e(number_format($total, 2)); ?></span>
                    </div>
                </div>
            </div>
        </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Arfa\Desktop\ProjectDreamPC\DreamPC\resources\views/checkout/index.blade.php ENDPATH**/ ?>