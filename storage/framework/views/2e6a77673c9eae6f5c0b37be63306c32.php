<div class="mt-4 bg-slate-950/90 border border-slate-800 rounded-xl p-4 shadow-xl space-y-4">
    ...
    <div class="flex items-center justify-between border-b border-slate-800 pb-3">
        ...
        <div class="text-right">
            ...
            <span class="text-sm font-bold text-emerald-400">$<?php echo e(number_format($totalPrice ?? 0, 2)); ?></span>
        </div>
    </div>
            ...
            <div class="bg-slate-900/80 border border-slate-800/80 hover:border-blue-500/40 p-3 rounded-lg flex items-center space-x-3 transition">
                <div class="w-12 h-12 rounded-md bg-slate-950 border border-slate-800 flex items-center justify-center overflow-hidden flex-shrink-0">
                    <?php if(!empty($product->image_path)): ?>
                        <img src="<?php echo e($product->image_path); ?>" alt="<?php echo e($product->name); ?>" class="w-full h-full object-cover">
                    <?php else: ?>
                        <span class="text-lg">🖥️</span>
                    <?php endif; ?>
                </div>
                <div class="flex-grow min-w-0">
                    <h4 class="text-xs font-medium text-slate-200 truncate"><?php echo e($product->name); ?></h4>
                    <div class="flex items-center justify-between mt-1">
                        <span class="text-[10px] text-slate-400 font-mono"><?php echo e($product->brand); ?></span>
                        <span class="text-xs font-bold text-emerald-400">$<?php echo e(number_format($product->price, 2)); ?></span>
                    </div>
                </div>
            </div>
    ...
    <?php if(!empty($incompatibilities)): ?>
        <div class="bg-amber-500/10 border border-amber-500/20 p-2.5 rounded-lg text-xs text-amber-300 space-y-1">
            <?php $__currentLoopData = $incompatibilities; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $warning): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    <div class="pt-2 border-t border-slate-800/80 flex justify-end">
        <form method="POST" action="<?php echo e(route('cart.batch-add')); ?>" class="w-full sm:w-auto">
            <?php echo csrf_field(); ?>
            <?php $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<?php /**PATH C:\Users\Arfa\Desktop\ProjectDreamPC\DreamPC\resources\views/chat/partials/build-card.blade.php ENDPATH**/ ?>